...
$desde = $_GET['desde'] ?? '';
$hasta = $_GET['hasta'] ?? '';
$hasDesde = preg_match('/^\d{4}-\d{2}-\d{2}$/', $desde);
$hasHasta = preg_match('/^\d{4}-\d{2}-\d{2}$/', $hasta);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (in_array($nuevo, $ESTADOS, true)) {
    ...
    $up->bind_param("sii", $nuevo, $id, $negocio_id);
    $up->execute();
    $up->close();
  }
  $params = [];
  if ($estadoVal) $params['estado'] = $estado;
  if ($hasDesde)  $params['desde']  = $desde;
  if ($hasHasta)  $params['hasta']  = $hasta;
  $qs = http_build_query($params);
  header("Location: pedidos.php" . ($qs ? "?$qs" : ""));
  exit;
}
...
function badgeClass($estado)
{
  switch ($estado) {
    ...
    case 'En preparación':
      return 'badge-status-preparacion';
    case 'Listo':
      return 'badge-status-listo';
    default:
      return 'bg-secondary';
  }
}
  ...
  <div class="container-fluid">
    <div class="row">
      ...
      <main class="col-md-9 p-4">
        ...
        <?php
        $q = [];
        if ($estadoVal) $q['estado'] = $estado;
        if ($hasDesde)  $q['desde']  = $desde;
        if ($hasHasta)  $q['hasta']  = $hasta;
        $q['export'] = 'csv';
        ...
        <form method="get" class="row g-2 mb-3">
          <div class="col-md-3">
            <select name="estado" class="form-select">
              ...
              <?php foreach ($ESTADOS as $e): ?>
                <option value="<?php echo $e; ?>" <?php echo ($estadoVal && $estado === $e) ? 'selected' : ''; ?>>
                  <?php echo $e; ?>
                </option>
              <?php endforeach; ?>
            </select>
          ...
          <div class="col-md-3">
            ...
            <a class="btn btn-outline-secondary ms-2" href="pedidos.php?<?php echo $qs; ?>">Exportar CSV</a>
          </div>
        </form>
