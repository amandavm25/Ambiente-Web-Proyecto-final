...
$st->bind_param("i",$negocio_id); $st->execute();
...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if ($direccion==='' || !is_array($items) || !count($items)){
    ...
    foreach($items as $it){
      ...
      $q->bind_param("ii",$pid,$negocio_id); $q->execute();
      $row=$q->get_result()->fetch_assoc(); $q->close();
      if(!$row){ $msg_err="Producto no disponible"; break; }
      $precio=(float)$row['Precio']; $sub=$precio*$qty;
      $subtotal += $sub; $lineas[] = [$pid,$qty,$sub];
    }
    ...
    if(!$msg_err && $cupon_in!==''){
      $c=$mysqli->prepare("
        ...
        LIMIT 1
      ");
      $c->bind_param("si",$cupon_in,$negocio_id); $c->execute();
      $cup=$c->get_result()->fetch_assoc(); $c->close();

      if($cup){
        // El descuento se aplica sobre el subtotal (no sobre el envío)
        if($cup['Tipo']==='porc'){ $descuento = round($subtotal * ((float)$cup['Valor']/100), 2); }
        else                     { $descuento = (float)$cup['Valor']; }
    ...
    if(!$msg_err){
      try{
        ...
        $estado='Pendiente'; $fecha=date('Y-m-d H:i:s');
...
$ins->close();
        ...
        foreach($lineas as [$pid,$qty,$sub]){ $det->bind_param("iiid",$pedido_id,$pid,$qty,$sub); $det->execute(); }
        $det->close();
        ...
        if($cupon_cod){
          ...
          $u->bind_param("s",$cupon_cod); $u->execute(); $u->close();
        }
      ...
      <?php if($msg_ok): ?>
        ...
        <script>
          (function(){
            ...
            const all=_getAll(); delete all["<?php echo $negocio_id; ?>"]; _saveAll(all);
            ...
            <div id="cartBox" style="display:none">
              <div class="table-responsive">
                <table class="table table-sm align-middle">
                  <thead><tr><th>Producto</th><th class="text-center">Cant.</th><th class="text-end">Subtotal</th></tr></thead>
                  <tbody id="cartRows"></tbody>
                  <tfoot>
                    <tr><th colspan="2" class="text-end">Subtotal</th><th class="text-end" id="cartSubtotal">₡0</th></tr>
                    <tr><th colspan="2" class="text-end">Descuento</th><th class="text-end" id="cartDesc">₡0</th></tr>
                    <tr><th colspan="2" class="text-end">Envío</th><th class="text-end" id="cartEnvio">₡0</th></tr>
                    <tr><th colspan="2" class="text-end">Total</th><th class="text-end" id="cartTotal">₡0</th></tr>
                  </tfoot>
                </table>
              </div>
...
<script>
  ...
  function renderCart(){
    ...
    rows.innerHTML='';
    if(!items.length){ cartBox.style.display='none'; cartEmpty.style.display='block'; calcTotals(); return; }
    cartBox.style.display='block'; cartEmpty.style.display='none';
  ...
  function changeQty(id,delta){
    const items=getCartFor(NEGOCIO); const i=items.findIndex(x=>x.id==id);
    if(i>=0){ items[i].qty+=delta; if(items[i].qty<=0) items.splice(i,1); setCartFor(NEGOCIO,items); renderCart(); }
  }
  function calcTotals(){
    const items=getCartFor(NEGOCIO)||[];
    let subtotal=0; for(const it of items){ subtotal+=(Number(it.price)||0)*(Number(it.qty)||0); }
    const envio = (selEntrega?.value==='Recoger en tienda') ? 0 : ENVIO_NEG;
    // El descuento solo se calcula definitivo en servidor (por fechas/uso/negocio). Aquí se muestra 0 como indicativo.
    const desc = 0;
  ...
  document.getElementById('btnVaciar')?.addEventListener('click',()=>{ if(confirm('¿Vaciar carrito?')){ clearCartFor(NEGOCIO); renderCart(); } });

  document.getElementById('checkoutForm').addEventListener('submit',(e)=>{
    const items=getCartFor(NEGOCIO)||[];
    if(!items.length){ e.preventDefault(); alert('Tu carrito está vacío.'); return; }
    document.getElementById('items_json').value = JSON.stringify(items.map(x=>({id:x.id, qty:x.qty})));
  });
